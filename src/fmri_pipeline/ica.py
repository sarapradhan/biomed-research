...
def run_subject_spatial_ica(clean_bold_file: str, mask_file: str, cfg: Dict) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Uses proper spatial ICA: the data matrix is transposed so that voxels are
    treated as observations (samples) and timepoints as features. FastICA then
    finds maximally non-Gaussian spatial patterns directly. This matches the
    FSL MELODIC / nilearn CanICA convention for fMRI spatial ICA.

    Spatial ICA decomposition:
        X_transposed [vox, time] -> FastICA -> sources [vox, n_comp] = spatial maps
        mixing_ [time, n_comp] = timecourses (one per component)
    """
    ...
    # Transpose: [vox, time] so voxels are samples → ICA finds spatial sources
    spatial_sources = ica.fit_transform(x.T)   # [vox, n_comp]
    maps = spatial_sources.T                    # [n_comp, vox]
    timecourses = ica.mixing_                   # [time, n_comp]
