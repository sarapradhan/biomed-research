"""Voxelwise regional homogeneity (ReHo) via Kendall's coefficient of concordance.

ReHo quantifies the similarity of time courses among neighboring voxels
using Kendall's W over a 26-voxel neighborhood. The computation is
parallelized across spatial chunks using joblib. Output maps are optionally
z-score normalized within the gray-matter mask.

References
----------
Zang, Y. F. et al. (2004). Regional homogeneity approach to fMRI data
...
def _kendalls_w(nei_ts: np.ndarray) -> float:
    ...
    if m < 3 or n < 10:
        return 0.0
    ranks = rankdata(nei_ts, axis=1)  # axis=1: rank timepoints within each voxel (Kendall's W)
