...
Provides pairwise Pearson correlation with Fisher z-transform for static FC,
sliding-window temporal variability for dynamic FC, and optional k-means
state clustering of windowed connectivity matrices.
"""
...
def static_fc(roi_ts: np.ndarray) -> np.ndarray:
    ...
    ----------
    roi_ts : np.ndarray
        ROI time series matrix of shape (n_timepoints, n_rois).
